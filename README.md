...
Provides a Docker image for OpenSearch with the `repository-s3`, `prometheus-exporter` & `telemetry-otel` plugin installed by default. This enables native backups and snapshots of OpenSearch clusters to Amazon S3 or S3-compatible endpoints without requiring custom startup scripts or init containers.
